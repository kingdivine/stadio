...
var ViewModel = function(){ 
    ...
    self.currentTeam().marker.setAnimation(google.maps.Animation.BOUNCE);
    setTimeout(function(){self.currentTeam().marker.setAnimation(null); }, 850);
    ...
    var populateWindow = function(yelpData){
        windowContent = '<h3>'+ self.currentTeam().teamName +'</h3>' +
                            ...
                            '<a href="' + self.currentTeam().wikiUrl + '" target="blank">' + '<br> More Info</a>';
        if (yelpData != false){
            ...
            windowContent = windowContent + '<p> Failed to load Yelp Data. Reopen to try again.</p>';
        }
    ...
    self.searchResults = ko.computed(function(){
        ...
        for (var i = 0; i < self.teamList().length; i++) {
            ...
            if (teamName.indexOf(searchTerm) != -1){
                ...
                self.teamList()[i].marker.setVisible(true);                             
            }
            else{
                self.teamList()[i].marker.setVisible(false);
            }
        };
        return results;     
    })
...
var googleFail = function(){
    alert('Map failed to load. Please refresh page.');
};
